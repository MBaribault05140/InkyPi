...
class Weather(BasePlugin):
    ...
    def generate_image(self, settings, device_config):
        ...
        ow_visibility = self.get_current_weather_visibility(AIR_QUALITY_API_KEY)
        ...
        template_params = self.parse_weather_data(weather_data, aqi_data, ow_visibility, tz)
    ...
    def get_current_weather_visibility(self, api_key):
        ...
        if response.ok:
            data = response.json()
            return data.get("visibility", 10000) / 1609.34  # meters to miles
        return 10  # fallback to max visibility
