...
class WeatherPlugin:
    ...
    def parse_forecast(self, daily_forecast, tz):
        ...
        for day in daily_forecast:
            ...
            # Debugging: Print the high and low temperatures to ensure they are being captured correctly
            print(f"High: {high}, Low: {low}")  # Debugging line to check values
            
            forecast.append({
                "day": dt.strftime("%a"),
                "high_temp": high,  # Pass 'high_temp' to match HTML template
                "low_temp": low,    # Pass 'low_temp' to match HTML template
